...
Изход
Да се отпечата на конзолата колко ще са разходите на Джеси, ако започне да спортува баскетбол. Сумата да
бъде форматирана до втория знак след десетичния знак. */

function basketballEquipment(input) {
    let yearTaxPrice = Number(input.shift());

    let sneakersPrice = yearTaxPrice - (yearTaxPrice*0.40);
    let sweatSuitPrice = sneakersPrice - (sneakersPrice*0.20);
    let ballPrice = sweatSuitPrice / 4;
    let accessoriesPrice = ballPrice / 5;

    let totalCosts = yearTaxPrice + sneakersPrice + sweatSuitPrice + ballPrice + accessoriesPrice;
