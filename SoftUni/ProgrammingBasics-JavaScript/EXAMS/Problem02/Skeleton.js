...
Трябва да се има предвид, че поради наклона на улея, на всеки 120 метра неговото време намаля с 2.5
секунди.
Вход
...
function skeleton(input) {
    ...
    let secondOnControl = Number(input.shift());
    let uleiLength = Number(input.shift());
    let travelSeconds = Number(input.shift());

    let controlInSeconds = (controlMinutes * 60) + secondOnControl;
    let timeDown = uleiLength / 120;
    let totalDownTime = timeDown * 2.50;
    let marinTime = (uleiLength / 100) * travelSeconds - totalDownTime;
    ...
    if (marinTime <= controlInSeconds) {
        ...
        let slowerSeconds = marinTime - controlInSeconds;
        console.log(`No, Marin failed! He was ${slowerSeconds.toFixed(3)} second slower.`);
    }
